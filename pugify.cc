...
#include "3rd/lodepng.cpp"
#include "3rd/jpge.h"
#include "3rd/jpge.cpp"
...
image decode_png( const void *ptr, unsigned size ) {
    int w = 0, h = 0, bpp = 0;
    stbi_uc *imageuc = stbi_load_from_memory( (const unsigned char *)ptr, size, &w, &h, &bpp, 4 );
    if( imageuc ) {
        // convert to image class
        image img { (unsigned)w, (unsigned)h, std::vector<unsigned char>( w * h * 4 ) };
...
image decode_jpg( const void *ptr, unsigned size ) {
    return decode_png( ptr, size );
}

image decode_pug( const void *ptr, unsigned size ) {
...
std::string encode_png( unsigned w, unsigned h, const void *data, unsigned stride ) {
    if( w && h && data && stride ) {
        ...
        size_t pngsize;
        unsigned bpp = 8;
        unsigned error = lodepng_encode_memory( &png, &pngsize, (const unsigned char *)data, w, h, mode, bpp );
        if( !error && pngsize ) {
            std::string buf;
            buf.resize(pngsize);
